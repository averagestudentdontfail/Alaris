...
The Alaris system employs the QD+ approximation method of @andersen2016 as the first stage of boundary estimation. This method provides an accurate quasi-analytic approximation that serves as initialisation for subsequent refinement.
...
The Alaris system employs the FP-B' stabilised iteration scheme from @healy2021 to prevent oscillatory behaviour in long-maturity options.
...
The signal generation system implements the criteria from @atilgan2014, expressed as formal predicates.
...
The Alaris system employs the @yang2000 estimator for realised volatility, which is efficient for OHLC (open-high-low-close) data and robust to opening gaps.
...
Let $\{\sigma_{I,i}^{(F)}\}_{i=1}^{N}$ and $\{\sigma_{I,i}^{(B)}\}_{i=1}^{N}$ be historical IV observations for front and back months.
...
The underlying asset price follows a continuous-time stochastic process (@eq-sde). **This is not a fault**—it is the defining characteristic of the problem domain. The system handles this through:
...
The Alaris system uses $N = 50$ collocation points, providing accuracy exceeding the @healy2021 benchmark values.
...
The implementation is validated against @healy2021 Table 2 benchmark values:
