...
**Violations**: 3 instances in `CA501A.cs`

### 1. CA501A.cs:188 - CalculateVega
```csharp
catch (Exception ex)
{
    return 0.0;
...
### 2. CA501A.cs:226 - CalculateTheta
```csharp
catch (Exception ex)
{
    return 0.0;
...
**Action**: Same pattern as CA301A fixes - identify expected failure modes and catch only those.
...
**Location**: `CA504A.cs:1` (entire namespace)
...
**Location**: `CA501A.cs:57`

**Issue**: Implicit operator without named method

**Violation**:
```csharp
public static implicit operator PricingEngine(CA501A engine)
```

**Fix**: Add named method:
```csharp
public static implicit operator PricingEngine(CA501A engine) => ToCA501A(engine);

public static PricingEngine ToCA501A(CA501A engine)
{
...
**Location**: `CA503A.cs:577` (2 instances)
...
1. **Fix 3 Generic Exception Catches** (CA1031) - 1-2 hours
   - CA501A.cs: CalculateVega, CalculateTheta, CalculateRho
