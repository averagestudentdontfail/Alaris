...
|----------|-----------|--------|---------------------|--------|
| CRITICAL | CA301A | Greek Calculations | 143/option | 5-10 |
| CRITICAL | CA503A | RefineUsingFpbPrime | 200+ arrays | 5-10 |
| HIGH | CA108A | CalculateRolling | 252+ lists | 2-3 |
| MEDIUM | CA106A | Analyze | 3 arrays | 0 heap |
| MEDIUM | CA111A | Various LINQ | Multiple | Minimal |

---

## Critical Hot Path #1: Greek Calculations

**File**: `Alaris.Strategy/Bridge/CA301A.cs`
...
```csharp
private static readonly ObjectPool<CA312As> _pool =
    new DefaultObjectPool<CA312As>(new ParameterPolicy());

private CA312As ClonePooled(CA312As original)
{
    var clone = _pool.Get();
...
**File**: `Alaris.Strategy/Core/CA108A.cs`
...
### CA106A.Analyze - Use Span<T>

```csharp
public CA106AAnalysis Analyze(IReadOnlyList<CA106APoint> points)
{
...
### CA111A - Eliminate LINQ Materialize
...
| Task | File | Effort |
|------|------|--------|
| Implement QuantLib infrastructure caching | CA301A.cs | 2 days |
| Add CA312As pooling | CA301A.cs | 0.5 days |
...
| Task | File | Effort |
|------|------|--------|
| Add ArrayPool to Kim solver | CA503A.cs | 1 day |
| Eliminate inner loop cloning | CA503A.cs | 0.5 days |
...
| Task | File | Effort |
|------|------|--------|
| Implement circular buffer for CA108A | CA108A.cs | 1 day |
| Add Span<T> to CA106A | CA106A.cs | 0.5 days |
| Remove LINQ materialize in CA111A | CA111A.cs | 0.5 days |
...
2. **New file**: `Alaris.Strategy/Infrastructure/CA312AsPool.cs`
3. **Modified**: `CA301A.cs` (Greek calculations)
4. **Modified**: `CA503A.cs` (ArrayPool integration)
5. **Modified**: `CA108A.cs` (circular buffer)
