...
public sealed class DBEX001A
{
    ...
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown when spot or strike is non-positive.
    /// </exception>
    ...
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown when model value is negative or parameters invalid.
    /// </exception>
        ...
        double timeToExpiry)
    {
        ...
        blendedValue = Math.Max(blendedValue, intrinsicValue);
    ...
    /// <remarks>
    /// <para>
    /// Near expiry, Greeks have special limiting behaviour:
    /// </para>
    /// <list type="bullet">
    /// <item>Delta → 0 (OTM) or 1 (ITM) for calls; reverse for puts</item>
    /// <item>Gamma → 0 (except approaching infinity at ATM)</item>
    /// <item>Vega → 0</item>
    /// <item>Theta → remaining time value / τ</item>
    /// </list>
    /// </remarks>
    /// <param name="spot">Current spot price.</param>
    /// <param name="strike">Strike price.</param>
        ...
        double timeToExpiry)
    {
        ...
        bool isATM = Math.Abs(moneyness - 1.0) < 0.01; // Within 1%
        ...
        // We cap it at a large but finite value for numerical stability
        double gamma;
        if (isATM && timeToExpiry > 0)
        {
            // Approximate ATM gamma near expiry: Γ ≈ 1 / (S × σ × √τ)
            // Use a reasonable vol estimate (30%) for capping
            const double VolEstimate = 0.30;
            gamma = Math.Min(1.0 / (spot * VolEstimate * Math.Sqrt(timeToExpiry)), 100.0 / spot);
        }
        else
        {
            gamma = 0.0;
        ...
        // Vega approaches zero (no time for vol to matter)
        double vega = 0.0;
